  ...
  try {
    ...
    if (events.length === 0) {
      return res.status(404).json({
        success: false,
        message:
          'Event not found. This QR code or link may be invalid.',
      });
    }
...
const createEvent = async (req, res) => {
  ...
  try {
    ...
    if (department_ids && department_ids.length > 0) {
      const deptValues = department_ids.map((dept_id) => [
        event_id,
        dept_id,
      ]);
...
const getEventQR = async (req, res) => {
  try {
    ...
    // IMPORTANT:
    // event_id MUST be selected because it is used to create
    // the public /checkin/:eventId URL.
    ...
    // Build the public frontend URL
    const frontendURL =
      process.env.FRONTEND_URL || 'http://localhost:5173';

    // Remove trailing slash if FRONTEND_URL has one
    const cleanFrontendURL = frontendURL.replace(/\/$/, '');

    // IMPORTANT:
    // The QR code now points to the React route:
    //
    // /checkin/:eventId
    //
    // Example:
    // https://eventhub.vercel.app/checkin/1
    //
    // Your App.jsx already contains:
    // <Route path="/checkin/:eventId" element={<CheckIn />} />
    const eventLink = `${cleanFrontendURL}/checkin/${rows[0].event_id}`;
